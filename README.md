...
中国移动 CMCC XR30 固件。基于官方 ImageBuilder 构建，不维护独立源码 fork；以 62 行增量设备树叠加上游配置，其余软件包与内核直接同步官方上游。

## 架构

| 组件 | 上游 | 适配实现 |
|---|---|---|
| 系统 / 内核 | [ImmortalWrt 官方](https://github.com/immortalwrt/immortalwrt) | 官方 ImageBuilder 构建，直接使用官方预编译内核与软件仓库 |
| 设备树 | 上游 RAX3000M (NAND) | 基底 DTB + [62 行增量 overlay](dts/mt7981b-cmcc-xr30-nand.dtso)，构建期 `fdtoverlay` 链式合成 |
| 网络 / 升级 | 上游 `02_network` / `platform.sh` | 构建期拉取上游脚本，注入 `cmcc,xr30*` 的 1 处网络与 3 处升级匹配规则 |
...
- 网口与扩展：1× GE WAN (eth1)、3× GE LAN (MT7531AE 直通)、1× USB 3.0 (UAS，支持 ext4/vfat/exFAT 自动挂载)
- 无线：2.4G 574M + 5G 2402M @ 160MHz (mt76 开源驱动)
- 转发加速：MTK PPE 硬件流控 (HNAT)、Packet Steering 多核分发、TCP BBR
- 管理界面：LuCI (简体中文)、Argon 主题、TTYD 网页终端

## 刷写与登录

1. 在 [bl-mt798x-xr30](https://github.com/RSxiaoyu/bl-mt798x-xr30) Web 恢复控制台 (`192.168.1.1`) 上传 `*sysupgrade.itb` 刷入；或在已运行系统中执行 `sysupgrade -n *.itb`。
2. 刷入完成后访问后台：`http://192.168.1.1`（用户名：`root`，默认无密码）。
