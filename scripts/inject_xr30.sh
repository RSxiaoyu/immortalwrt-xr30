...
MOMO_JSON=$(curl -sLf --retry 3 "$MOMO_URL/index.json" || true)
...
if [ -n "$MOMO_VER" ]; then
    curl -sLf --retry 3 "$MOMO_URL/momo-${MOMO_VER}.apk" -o "packages/momo-${MOMO_VER}.apk"
fi
if [ -n "$LUCI_MOMO_VER" ]; then
    curl -sLf --retry 3 "$MOMO_URL/luci-app-momo-${LUCI_MOMO_VER}.apk" -o "packages/luci-app-momo-${LUCI_MOMO_VER}.apk"
fi
if [ -n "$LUCI_ZH_VER" ]; then
    curl -sLf --retry 3 "$MOMO_URL/luci-i18n-momo-zh-cn-${LUCI_ZH_VER}.apk" -o "packages/luci-i18n-momo-zh-cn-${LUCI_ZH_VER}.apk"
fi
...
curl -sLf --retry 3 "$SINGBOX_APK_URL" -o "packages/sing-box-${SINGBOX_VER}-r0.apk" || true
